...
def test_unsupported_extension():
    with tempfile.NamedTemporaryFile(
        "w+", suffix=".unsupported", delete=False
    ) as f:
        f.write("test")
        f.flush()
        path = f.name
...
def test_changed_value():
    ...
    diff = diff_builder.build_diff(d1, d2)
    assert diff == [
        {"key": "a", "status": "changed", "old_value": 1, "new_value": 2}
    ]
...
def test_format_diff_output_simple():
    diff = [
        ...
        {"key": "d", "status": "unchanged", "value": 5},
        {
            "key": "e",
            "status": "nested",
            "children": [{"key": "f", "status": "added", "value": 6}],
        },
    ]
...
def test_main_stylish_and_plain(monkeypatch, tmp_path, capsys):
    ...
    # stylish
    monkeypatch.setattr(
        sys, "argv", ["gendiff", str(file1), str(file2), "--format", "stylish"]
    )
    gendiff.main()
    ...
    # plain
    monkeypatch.setattr(
        sys, "argv", ["gendiff", str(file1), str(file2), "--format", "plain"]
    )
    gendiff.main()
    out, _ = capsys.readouterr()
...
def test_main_with_json(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        sys, "argv", ["gendiff", str(file1), str(file2), "--format", "json"]
    )
    gendiff.main()
    out, _ = capsys.readouterr()
